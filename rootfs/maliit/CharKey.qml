...
Item {
    ...
    // Draw the key as pressed even when it is not being touched. Used by the
    // latching modifier keys so a held ctrl/alt stays visibly engaged.
    ...
    Item {
        ...
        ToolButton {
            ...
            down: keyMouseArea.pressed || key.forceDown
    ...
    PressArea {
        ...
        onReleased: {
            ...
            if (extendedKeysShown) {
                ...
                // A latched ctrl/alt turns this key into a shortcut rather than
                // text: send it as a key sequence so the app sees a real
                // Ctrl+C, and drop the latch afterwards.
                if (panel.modsActive()) {
                    var seqBase = keyToSend;
                    if (action === "return")
                        seqBase = "Return";
                    else if (action === "space")
                        seqBase = "Space";
                    else if (seqBase.length === 1)
                        seqBase = seqBase.toUpperCase();

                    panel.sendWithMods(seqBase);
                    keySent(keyToSend);
                    return;
                }
