...
| Suspend / resume / idle sleep | working | s2idle works, touchscreen survives it, power button wakes. Fedora ships `sleep.target`/`suspend.target` **masked**, which makes logind report "Access denied"; and PowerDevil ignores its profile config entirely, so idle sleep is handled by `tools/tabs6-idled.py` - see [`docs/SLEEP.md`](docs/SLEEP.md) |
