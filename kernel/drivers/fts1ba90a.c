				     ...
				     int cmd_len, int delay_ms)
{
	...
	for (;;) {
		...
		if (retry++ > FTS_RETRY_COUNT * 25) {
			 ...
			 * already proven the part alive, so treat a missing
			 * echo as informational and carry on.
			 */
			dev_warn(&ts->client->dev,
				 "no echo for cmd %02x (ignored): %*ph\n",
				 cmd[0], 8, data);
			return 0;
		}
		msleep(20);
